...
terminalRadioInput.forEach(radio => {
  radio.addEventListener('click', function () {
    ...
    if (this.oldChecked) this.checked = false;
...
form.addEventListener('submit', e => {
  ...
  if (Boolean(+shop.value)) {
    let shopNumber =
      ip.value == '' ? shop.value : shop.value + '_' + ip.value.slice(-3);
...
function statusCountKey(keys) {
  ...
  countKey > 1
    ? statusCountKeys.classList.add('more')
    : (statusCountKeys.classList = 'key-status');
}
...
const url = 'https://prro-configurator.onrender.com';
